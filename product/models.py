...
class Variant(models.Model):
    ...
    def __str__(self):
        return f'{self.size} Size of {self.product.name}'
    
class VariantColour(models.Model):
    variant = models.ForeignKey(Variant, on_delete=models.CASCADE, related_name='colours')
    colour = models.CharField(max_length=255)
    stock = models.PositiveBigIntegerField(default=0)
    
    def __str__(self):
        return f'{self.colour} Colour of {self.variant.size} Size {self.variant.product.name}'
